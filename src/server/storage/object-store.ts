 ...
 * stünde hier ein anderer Treiber, würde jede hochgeladene Datei in einen
 * Speicher geschrieben und aus einem anderen gelesen, und die Aufbereitung
 * schlüge stumm fehl.
 */
 ...
 *
 * Objekte liegen ausschließlich in Vercel Blob. Ein abweichender Wert stammt
 * aus einer Fassung, die dieser Speicher nicht lesen kann; der laute Fehler
 * ist besser als ein stiller Griff in den falschen Speicher.
 */
export function createPrivateObjectStore(
  storageDriver = resolveStorageDriver(),
): PrivateObjectStore {
  if (storageDriver !== "vercel-blob") {
    throw new Error(`UNSUPPORTED_STORAGE_DRIVER: ${storageDriver}`);
  }
