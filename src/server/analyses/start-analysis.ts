...
export async function startAnalysis(input: AnalysisStartInput): Promise<StartAnalysisResult> {
  ...
  // Optional und außerhalb der Transaktion: ein Jev-Schlüssel wird beim Anbieter
  // geprüft, und ohne ihn läuft der Start unverändert (`off`).
  const jev = await connectAnalysisJevAssist(boundDraft.id);
    ...
    result = await db.transaction(async (transaction) => {
      ...
      await appendAuditEvent(transaction, {
        ...
        metadata: {
          ...
          analysisProfile: scope.analysisProfile,
          requirementCount: selectedRequirements.length,
          // Nur die eingefrorene Stufe, nie ein Schlüssel.
          jevAssistMode: jev.mode,
        },
      });
  ...
  // Ein gleichzeitig entstandener Lauf besitzt seinen eigenen Jev-Schlüssel.
  if (result.reused) await discardJevCredential();
